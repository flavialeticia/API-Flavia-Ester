...
def api_create_user(payload):
    return send("POST", "/users", json_data=payload)
...
def api_post_message(headers, titulo, conteudo):
    ...
    return send("POST", "/messages", json_data=payload, headers=headers), payload

def api_put_message(headers, mid, **payload):
    return send("PUT", f"/messages/{mid}", json_data=payload, headers=headers), payload

def api_get_message(headers, mid):
    return send("GET", f"/messages/{mid}", headers=headers)

def api_delete_message(headers, mid):
    return send("DELETE", f"/messages/{mid}", headers=headers)

def api_post_comment(headers, mid, conteudo):
    payload = {"conteudo": conteudo}
    return send("POST", f"/messages/{mid}/comments", json_data=payload, headers=headers), payload

def api_put_comment(headers, mid, cid, **payload):
    return send("PUT", f"/messages/{mid}/comments/{cid}", json_data=payload, headers=headers), payload

def api_post_curtir(headers, mid):
    return send("POST", f"/messages/{mid}/curtir", headers=headers)

def api_get_excluidas(headers):
    return send("GET", "/messages/excluidas", headers=headers)
...
def main():
    ...
    # 2) USER1 cria três messages - (2pt 2pt 2pt)
    mids = []
    for i in range(3):
    ...
    (resp, url, met), payload = api_put_message(h2, target_mid, titulo="x", conteudo="tentativa user2") if target_mid else ((None, f"{BASE_URL}/messages/None", "PUT"), {})
    ok = status(resp) == 403
    print_test_block("7) USER2 tenta editar mensagem", met, url, h2, payload, resp, "403 Forbidden", ok)
    orig.add(3, ok)

    # 4) USER2 cria comentário - 5pt (201)
    (resp, url, met), payload = api_post_comment(h2, target_mid, "Comentário do USER2") if target_mid else ((None, f"{BASE_URL}/messages/None/comments", "POST"), {"conteudo": "Comentário do USER2"})
    ...
    (resp, url, met), payload = api_put_comment(h2, target_mid, comm_id, conteudo="Comentário do USER2 (editado)") if comm_id else ((None, f"{BASE_URL}/messages/{target_mid}/comments/None", "PUT"), {"conteudo": "Comentário do USER2 (editado)"})
    ok = status(resp) == 200
    ...
    resp, url, met = api_delete_message(ha, del_mid) if del_mid else (None, f"{BASE_URL}/messages/None", "DELETE")
    ok = status(resp) == 204
    ...
    resp, url, met = api_get_message(h1, del_mid) if del_mid else (None, f"{BASE_URL}/messages/None", "GET")
    ok = status(resp) == 404
    ...
    resp, url, met = api_get_message(h1, like_mid) if like_mid else (None, f"{BASE_URL}/messages/None", "GET")
    ...
    (resp_put, url_put, met_put), payload = api_put_message(h1, like_mid, conteudo="mudando conteúdo", curtidas=99) if like_mid else ((None, f"{BASE_URL}/messages/None", "PUT"), {"conteudo": "mudando conteúdo", "curtidas": 99})
    ...
    resp_chk, url_chk, met_chk = api_get_message(h1, like_mid) if like_mid else (None, f"{BASE_URL}/messages/None", "GET")
    ...
    # 10) POST /messages/{id}/curtir incrementa — 5 pts (USER2 curte)
    resp, url, met = api_post_curtir(h2, like_mid) if like_mid else (None, f"{BASE_URL}/messages/None/curtir", "POST")
    ...
    resp, url, met = api_post_curtir(ha, like_mid) if like_mid else (None, f"{BASE_URL}/messages/None/curtir", "POST")
    ok = status(resp) == 403
    ...
    resp, url, met = api_post_curtir(h1, like_mid) if like_mid else (None, f"{BASE_URL}/messages/None/curtir", "POST")
    ok = status(resp) == 403
    print_test_block("ALT 5) Autor tentar curtir própria mensagem", met, url, h1, None, resp, "403 Forbidden", ok)
    alt.add(5, ok)

    # 13) /messages/excluidas — 10 pts (ADMIN) contém deletada
    ...
    print_test_block("ALT 6) GET /messages/excluidas (ADMIN)", met, url, ha, None, resp, "200 OK + lista contém mensagem deletada", ok)
    alt.add(10, ok)

    # 14) somente admin pode acessar excluídas — 5 pts (USER → 403)
    resp, url, met = api_get_excluidas(h2)
    ok = status(resp) == 403
    print_test_block("ALT 7) /messages/excluidas com USER", met, url, h2, None, resp, "403 Forbidden", ok)
    alt.add(5, ok)
